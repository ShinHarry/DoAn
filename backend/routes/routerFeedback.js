...
router.post("/", verifyToken, authPage(["cus"]), async (req, res) => {
  try {
    ...
    if (!comment || comment.length < 30) {
      return res
        .status(400)
        .json({ message: "Phản hồi phải có ít nhất 30 ký tự" });
    }
    ...
    res
      .status(201)
      .json({ message: "Đã thêm bình luận thành công", feedback: newFeedback });
  } catch (error) {
