...
router.post("/", async (req, res) => {
  try {
    ...
    if (!productId || !name || typeof price !== "number" || !image) {
      return res
        .status(400)
        .json({ message: "Thiếu thông tin hoặc sai định dạng" });
    }
    ...
    if (existed) {
      return res
        .status(400)
        .json({ message: "Sản phẩm đã tồn tại trong wishlist" });
    }
...
router.delete("/clear", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi server khi xoá wishlist", error: err.message });
  }
});
